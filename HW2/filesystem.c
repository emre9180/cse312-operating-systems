...
{
    DirectoryTable *dir = findDirectory(&superBlock.rootDirectory, dirName);
    if (dir == NULL)
    {
        printf("Directory not found: %s\n", dirName);
        return;
...
{
    deleteDirectoryHelper(fsBase, dirName, &superBlock.rootDirectory);
}
...
DirectoryTable *findDirectory(DirectoryTable *dirTable, const char *directoryName)
{
    if (strcmp(directoryName, "/") == 0) // Root directory
    {
        return &superBlock.rootDirectory;
